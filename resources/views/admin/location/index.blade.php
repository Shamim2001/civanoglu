<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="min-w-max">
                <a href="{{route('dashboard-create')}}" class="fullwidth-btn">Add New Location</a>
            </div>
        </div>
    </x-slot>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-4 py-2 text-center ">
                                    <a href="{{route('dashboard-edit', $location->id)}}" class="bg-blue-500 text-white px-4 py-2 text-xs rounded">Edit</a>
                                    <a href="{{route('single-location', $location->id)}}" target="_blank" class="bg-green-500 text-white px-4 py-2 text-xs rounded">View</a>

                                    <form onclick="return confirm('Are you sure you want to delete the location?')" action="{{route('dashboard-destroy', $location->id)}}" method="post" class="inline-block"> @csrf
